...
if __name__ == '__main__':
    ...
    def validate(_model,
        ...
        _summary_manager.display_audio(tag=f'Validation {_fname[0].numpy().decode("utf-8")}/prediction',
                                       mel=model_out['mel'][0])
        ...
        for j, pred_mel in enumerate(model_out['mel']):
            ...
            tar_value = _mel[j, :tar_lengths[j], :]
            _summary_manager.display_mel(mel=predval, tag=f'Test/{_fname[j].numpy().decode("utf-8")}/predicted')
            _summary_manager.display_mel(mel=tar_value, tag=f'Test/{_fname[j].numpy().decode("utf-8")}/target')
            _summary_manager.display_audio(tag=f'Prediction {_fname[j].numpy().decode("utf-8")}/target', mel=tar_value)
            _summary_manager.display_audio(tag=f'Prediction {_fname[j].numpy().decode("utf-8")}/prediction',
                                           mel=predval)
    ...
    for _ in t:
        ...
        if model.step % config_dict['prediction_frequency'] == 0 and (
                model.step >= config_dict['prediction_start_step']):
